...
router.post("/add", async (req, res) =>{
    ...
    try {
        ...
        const result = await client.db("sample_mflix").collection("theaters").insertOne(body);
        if (result) {
            res.json({
...
router.get('/', async (req, res) =>{
    ...
    try {
        await client.connect();
        const result = await client.db("sample_mflix").collection("theaters").find({}).limit(5).toArray();
...
router.get("/:id", async (req, res) =>{
    ...
    try {
        await client.connect();
        const result = await client.db("sample_mflix").collection("theaters").findOne({_id: new ObjectId(id)});
        if (result) {
            res.status(200).send(result)            
        } else {
...
router.post('/', async (req, res)=>{
    ...
    try {
        await client.connect();
        const result = await client.db("sample_mflix").collection("theaters").insertMany(body);
        if(result){
            res.status(201).json({
...
router.patch('/:id', async (req, res)=>{
    ...
    try {
        await client.connect();
        const result = await client.db("sample_mflix").collection("theaters").updateOne({_id: new ObjectId(id)},{$set:{title:body, year:body.year}});
        if(result){
            res.status(201).json({
...
router.delete('/:id', async (req, res)=>{
    ...
    try {
        await client.connect();
        const result = await client.db("sample_mflix").collection("theaters").deleteOne({_id: new ObjectId(id)});
        if(result){
            res.status(201).json({
