...
router.post("/add", async (req, res) =>{
    ...
    try {
        ...
        const result = await client.db("sample_mflix").collection("sessions").insertOne(body);
        if (result) {
            res.json({
                message:"Se inserto la sesion en la base de datos",result,data:body
            })           
        } else {
            res.send("No se pudo insertar la sesion")
        }
    } catch (error) {
...
router.get('/', async (req, res) =>{
    ...
    try {
        await client.connect();
        const result = await client.db("sample_mflix").collection("sessions").find({}).limit(5).toArray();
        res.status(200).send(result)
        if (result) {
            res.send(result);
        } else {
            res.status(404).send("No se encontraron sesiones para mostrar")
        }
    } catch (error) {
        console.log(error)
    }finally{
...
router.get("/:id", async (req, res) =>{
    ...
    try {
        ...
        if (result) {
            ...
            res.status(404).send("No se encontro sesion con el id ingresado")
        }
    } catch (error) {
        res.send(error)
    }finally{
...
router.post('/', async (req, res)=>{
    ...
    try {
        ...
        if(result){
            res.status(201).json({
                message: 'Se crearon las sesiones en la Base de Datos',
                result,
                //data: body
            });
        }else{
            res.status(400).send("No se creo la sesion");
        }
    }catch(e){
        console.log(e);
    }finally{
...
router.delete('/:id', async (req, res)=>{
    ...
    try {
        ...
        if(result){
            res.status(201).json({
                message: 'Se borro la sesion',
                result,
                //data: body
            });
        }else{
            res.status(400).send("No se actualizo la sesion");
        }
    }catch(e){
        console.log(e);
    }finally{
