...
router.post("/add", async (req, res) =>{
    ...
    try {
        ...
        const result = await client.db("sample_mflix").collection("comments").insertOne(body);
        if (result) {
            res.json({
...
router.get('/', async (req, res) =>{
    ...
    try {
        await client.connect();
        const result = await client.db("sample_mflix").collection("comments").find({}).limit(5).toArray();
